...
We report some controlled experiments to verify the theory. Lacking a canonical multiclass problem for text, we constructed a new 3029-way supervised task by using articles from Wikipedia, where the article topics are classes and sentences in these articles are data points. Representations will be tested on a random binary classification task involves two articles, and the labels of the data point is which of the two articles it belongs to.  A 10-way classification task is similarly defined. Datapoints for the test tasks will be held out when we training representations. The representation class ${\mathcal F}$ is a simple one based on Gated Recurrent Unit (GRU).

The supervised method for learning representations uses multiclass data from 3000+ (all except the held out classes) and uses the representation from the layer before the softmax output layer. 
...
The highlighted parts in the table show that the unsupervised representations compete well with the supervised representations on the average $k$-way classification task. 

Additionally, even though not covered by our theory, the representation also performs respectably on the full 3029-way multiclass problem.
Our theory also suggests that the mean (centroid) of the unsupervised representations in each class should be good classifiers for average $k$-way supervised tasks. We find this to be true for unsupervised representations, and surprisingly for supervised representations as well.
