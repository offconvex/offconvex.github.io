...
author: Sanjeev Arora, Hrishikesh Khandeparker, Orestis Plevrakis, and Nikunj Saunshi
visible: False
---
...
<p style="text-align:center;">
<img src="/assets/CURLframework.svg" width="60%" />
</p>
...
This should not be surprising and it only suggests that further progress would require making more assumptions than the above minimalistic ones. Our paper makes progress by showing that under the above framework, if the unsupervised loss happens to be small at the end of contrastive learning then the resulting representations perform well on downstream classification. 
...
The paper also has experiments on CIFAR image data. In both text and image settings we find that good classification is possible on downstream tasks with as fews as 5 labeled examples.
