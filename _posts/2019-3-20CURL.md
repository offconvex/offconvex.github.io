...
In domains such as computer vision, we know the answer is "yes" because deep convolutional neural networks (CNNs) trained to high accuracy on large multiclass labeled datasets such as [ImageNet](http://www.image-net.org/) end up learning very powerful and succinct representations along the way. 
The penultimate layer of the net --- the input into the final [softmax layer](https://en.wikipedia.org/wiki/Softmax_function) --- serves as a good semantic embedding of the image in new unrelated visual tasks. 
...
Earlier methods along such lines include *kernel learning* and *semi-supervised learning*, but there training typically requires at least a few labeled examples from the classification tasks that will be seen in future. 
Bayesian approaches using generative models are also well-established in simpler settings, but have proved difficult for complicated data such as images and text. 
...
Here we give a high level view of these results and invite you to read our [paper](https://arxiv.org/abs/1902.09229) for more details.
...
Finally, the framework provides guidelines for designing an unsupervised objective when *blocks* of similar data are available (e.g., sentences in a paragraph). 
...
To verify components of the theory, we performed controlled experiments by simulating the data generation process of the framework. 
Due to lack of a canonical multiclass problem for text, we constructed a 3029-way supervised task by using articles from Wikipedia, where the article topics are classes and sentences in these articles are data points. 
Similar data points are just pairs of sentences sampled from the same article. 
We learn representations using a simple GRU architecture by minimizing the above unsupervised loss.
The classifiers for a task are learnt using the available labeled data for that task.

<p style="text-align:center;">
<img src="/assets/CURLexperiment.svg" width="60%" />
</p>
The highlighted parts in the table show that the average $k$-way classification performance of the representations learnt using contrastive learning is very close to that of representations learnt using supervised learning.
Additionally, even though not covered by theory, it also performs respectably on the all-way multiclass problem.
Our theory also suggests that the mean (centroid) of the unsupervised representations in each class should be good classifiers for average $k$-way supervised tasks.
