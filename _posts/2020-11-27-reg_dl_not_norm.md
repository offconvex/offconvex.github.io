...
While (for $L \geq 2$) it is possible to constrain the rank of $W_{L : 1}$ by limiting dimensions of the parameter matrices $\\{ W_j \}$, from an implicit regularization standpoint, the case of interest is where rank is unconstrained (i.e. dimensions of $W_j$ are large enough for $W_{L : 1}$ to take on any value).
