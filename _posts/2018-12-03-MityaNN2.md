...
We start by stating an objective function that will be used to derive NNs for linear dimensionality reduction. Let $x_{t=1,\ldots,T}\in \mathbb{R}^n$ be a set of data points (inputs) and $y_{t=1,\ldots,T}\in \mathbb{R}^k$ ($k < n$) be their learned representation (outputs). The similarity of a pair of inputs, $x_t$ and $x_{t'}$, can be defined as their dot-product, $x_t^\top x_{t'}$. Analogously, the similarity of a pair of outputs is $y_{t}^\top y_{t'}$. Similarity matching, as its name suggests, learns a representation where the similarity between each pair of outputs matches that of the corresponding inputs: 
