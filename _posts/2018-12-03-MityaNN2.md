...
We start by stating an objective function that will be used to derive NNs for linear dimensionality reduction. Let ${ x}$$_{t=1,\ldots,T}\in \mathbb{R}^n$ be a set of data points (inputs) and ${ y}$$_{t=1,\ldots,T}\in \mathbb{R}^k$ ($k$ $<$ $n$) be their learned representation (outputs). The similarity of a pair of inputs, ${ x}$$_t$ and ${ x}$$_{t'}$, can be defined as their dot-product, ${ x}$$_t^\top$${ x}$$_{t'} $. Analogously, the similarity of a pair of outputs is ${ y}$$_{t}^\top$${ y}$$_{t'}$. Similarity matching, as its name suggests, learns a representation where the similarity between each pair of outputs matches that of the corresponding inputs: 
