...
Unlike non-convex optimization, sampling beyond log-concavity is very nascent. In this post, we will survey the basic tools and difficulties in beyond log-concavity. In the next post, we will survey recent progress in this direction, in particular with respect to handling multi-modality and manifold structure in the level sets, which will cover the papers [Simulated tempering Langevin Monte Carlo](https://arxiv.org/abs/1812.00793) by Rong Ge, Holden Lee, and Andrej Risteski and [Fast convergence for Langevin diffusion with matrix manifold structure](https://arxiv.org/abs/2002.05576) by Ankur Moitra and Andrej Risteski. 
...
As theorists, we'd like to develop theory that will lead to a better understanding of why and when these heuristics work. Much like we've done this for optimization -- we need to be guided both by hardness results and relevant structure in practice in this endeavour. 


The following table summarizes the comparisons we have come up with:

| Optimization: $\min_x f(x)$                                   | Sampling: $x\sim p(x)\propto e^{-f(x)}$                                       |
| ---------------------------------------------- | ----------------------------------------------- |
| **Convex**: Gradient descent finds global minimum. | **Log-concave**: Langevin Monte Carlo is efficient (enjoys rapid mixing). |
| **Non-convex**: $f$ can have many bad local minima. However, algorithms often work remarkably well in practice. | **Non-log-concave**: $p$ can be multimodal. Langevin diffusion can take exponential time, but temperature heuristics help. |
...
where B denotes Brownian motion of the appropriate dimensionality.

The crucial property of the above stochastic differential equation is that under fairly mild assumptions on $f$, the stationary distribution of the above process is $p(x) \propto e^{-f(x)}$. (If you're more comfortable with optimization, note that while gradient descent generally converges to (local) minima, the Gaussian noise term prevents LMC from converging to a single point - rather, it converges to a *stationary distribution*. See animation below.)   
...
> $$\text{Var}_p(g) \le C \int_{\mathbb R^d} ||\nabla g(x)||^2 p(x)\,dx.$$
...
|                                                           | Discrete (lazy) Markov chain                            | Langevin dynamics (Continuous process)                                      |
| --------------------------------------------------------- | ------------------------------------------------ | --------------------------------------------------------------------------- |
|                                                           | Laplacian $L=I-A$, where $A=$transition matrix                            | Infinitesimal generator $\mathcal L g = -\langle \nabla f,\nabla g\rangle +  \Delta g$                           |
| Dirichlet form                                            | $\mathcal E(g,g)=\langle g, Lg\rangle_p$, $g$ vector     | $\mathcal E(g,g) = -\int_{\mathbb R^d} g(x)\mathcal L g(x) p(x)=\int_{\mathbb R^d} \Vert \nabla g(x)\Vert^2p(x)\,dx$, $g$ function     |
| Poincaré inequality $\text{Var}_p(g)\le C\mathcal E(g,g)$ | $\text{Var}_p(g) \le C\langle g,Lg\rangle_p$ | $\text{Var}_p(g) \le C \int_{\mathbb R^d} \Vert\nabla g(x)\Vert^2 p(x)\,dx$ |
| Mixing                                                          | $\chi^2(p_t\Vert p) \le (1-\frac 1C)^t \chi^2(p_0\Vert p)$                                                 |  $\chi^2(p_t\Vert p) \le e^{-t/C} \chi^2(p_0\Vert p)$                                                                           |

To see how the Poincaré inequality represents a spectral gap in the discrete case, we write it in a more explicit form in a familiar special case. Let's assume that $p$ is the uniform distribution, and $v_1=\mathbf 1,\ldots, v_n$ are the eigenvectors of $A$ with eigenvalues $1=\lambda_1\ge \lambda_2\ge \cdots \ge \lambda_n\ge 0$, normalize $v_1,\ldots, v_n$ so they have unit norm with respect to $p$, i.e. $\Vert v_i\Vert^2=\frac 1n\sum_j v_{ij}^2=1$. 

Writing $g= \sum_i a_i v_i$, since $v_2,\ldots, v_n$ are orthogonal to $v_1=\mathbf 1$, we have $\frac{1}{n} \langle g, \mathbf{1} \rangle =  a^2_1$, so  $\text{Var}_p(g) = \frac{1}{n}(\sum_i  g_i^2) - a_1^2 = \sum_{i=2}^n a_i^2$. 
Also, $\langle g, Lg \rangle_p = \langle \sum_i a_iv_i, (I-A)(\sum_i a_iv_i)\rangle=  \sum_{i=2}^n a_i^2(1-\lambda_i)$. These coefficients are all at most $1-\lambda_2$, i.e. the *spectral gap*, so 
...
> **Theorem (Bakry, Emery 1985)**: If $p(x)$ is $\alpha$-strongly convex, then $p(x)$ satisfies a Poincaré inequality with constant $\frac1{\alpha}$.
