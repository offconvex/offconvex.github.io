...
In this formulation, there are several choices that we have to make as a GAN designer, and an important one is that of a loss function. One concrete choice for it is from the paper of Goodfellow et al. itself: the cross-entropy loss function.
...
Perhaps the simplest algorithm we can try for min-max optimization is gradient descent-ascent (GDA). Since the generator wants to minimize with respect to $x$ and the discriminator wants to maximize with respect to $y$, the idea is to do descent steps for $x$ and ascent steps for $y$. How to do this is not clear as this is a sequential optimization problem, and one strategy is to let the generator and discriminator alternate:

$$x_{i+1} = x_i -\nabla_x f(x_i,y_i),$$

$$y_{i+1} = y_i +\nabla_y f(x_i,y_i).$$

Other variants include, for instance, the so-called [optimistic mirror descent](https://arxiv.org/abs/1311.1869) (OMD) (see also [here](https://arxiv.org/abs/1807.02629) and  [here](https://arxiv.org/abs/1711.00141) for applications of OMD to GANs, and [here](https://arxiv.org/abs/1901.08511) for an analysis of OMD and related methods)
...
As for examples relevant to ML, when using GDA to train a GAN on a dataset consisting of points sampled from a mixture of four Gaussians in $\mathbb{R}^2$, we can see that GDA tends to cause the generator to cycle between two or more of these Gaussian modes. We also used GDA to train a GAN on the subset of the MNIST digits which have "0" or "1" as their label, which we refer to as the 0-1 MNIST dataset.  And we observed a cycling behavior for this dataset as well: After learning how to generate images of $0$'s, the GAN trained by GDA then forgets how to generate $0$'s for a long time and only generates $1$'s.

<div style="text-align:center;">
<img src="/assets/GDA_Gaussian.gif" alt="" />
<img src="/assets/GDA_MNIST.gif" alt="" />
<br>
...
*This means that decreasing $h$ requires us to decrease $f$ (converse is not true). So it indeed makes sense to move in the direction $-\nabla_x f(x,y)$!*
...
>
> *Step 2:* Compute $h(x+v, y) = f(x+v, w)$, by simulating a discriminator update $w$ via gradient ascent on $f(x+v, \cdot)$ with initial point $y$.
>
...
<div style="text-align:center;">
<img src="/assets/GDA_spiral_2.gif" alt="" />
<img src="/assets/OurAlgorithm_surface_run1.gif" alt="" />
<br>
