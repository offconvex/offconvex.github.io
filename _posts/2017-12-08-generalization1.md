...
[Dziugaite and Roy](https://arxiv.org/abs/1703.11008) take a slightly different tack. They start with McAllester's 1999 PAC-Bayes bound, which says that if the algorithm’s prior distribution on the hypotheses is $P$ then for every posterior distributions $Q$ (which could depend on the data) on the hypotheses the generalization error of the average classifier picked according to $Q$ is upper bounded as follows where $D()$ denotes [KL divergence](https://en.wikipedia.org/wiki/Kullback%E2%80%93Leibler_divergence):
