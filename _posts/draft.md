...
Of course experts will immediately ask: "Wasn't generalization theory invented precisely to address this issue?" For instance it shows how to add regularizers to the training objective to ensure the solution generalizes. Or, that *early stopping* (stopping before the optimum is reached) or even adding noise ---e.g. via stochastic gradient updates---  can be preferable to perfect optimization even in simple settings such as regression. 
